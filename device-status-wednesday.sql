import { CommunicationStatus, MotorCommunicationStatus } from "../../models";
import { SerialCategory } from "../../models/enum/serial-category.enum";
export const GET_DEVICE_STATUS_DETAIL = `DROP FUNCTION IF EXISTS get_device_detail(bigint,int,text,text);
  ...
  RETURN jsonb_build_object(    
          ...
          'weather', (
            ...
            WHERE s.device_id = d.id
              AND s.type = ${SerialCategory.WEATHER_STATION}
          ...
          'sensors', (
            ...
            FROM public.sensor AS s
            WHERE s.device_id = d.id
              AND s.type IN (${SerialCategory.SOIL_MOISTURE})
              AND s.status NOT IN (${CommunicationStatus.DELETED})
            LIMIT 1
          ),
          'water_level', (
            SELECT jsonb_agg(
            ...
            FROM public.sensor AS s
            WHERE s.device_id = d.id 
              AND s.type = ${SerialCategory.WATER_LEVEL}
            LIMIT 1
          )
    )
  FROM public.device AS d
  WHERE d.s_no = serial_no OR  d.name ILIKE serial_no || '%'
  LIMIT 1;
...
ELSIF user_mobile_number IS NULL AND (serial_no IS NOT NULL) AND device_type=${SerialCategory.SOIL_MOISTURE} THEN
    RETURN jsonb_build_object(    
      ...
      'sensors', (
        ...
        FROM public.sensor AS s
        WHERE (s.s_no =serial_no)
		and s.type=${SerialCategory.SOIL_MOISTURE}
		LIMIT 1
    ...
    'irrigation', (
        SELECT jsonb_build_object(
          ...
          'valves', (
                ...
                WHERE n.s_no = serial_no
                AND v.status NOT IN (${CommunicationStatus.DELETED})
            
            )
        )
        FROM public.node AS n
        WHERE n.s_no = serial_no
        AND n.type = ${SerialCategory.IRRIGATION_NODE}
        LIMIT 1
    )
)
FROM public.device AS d 
...
RETURN jsonb_build_object(
    ...
    'backwash_node',(
        ...
        AND n.type = ${SerialCategory.AUTO_BACKWASH}
        LIMIT 1 
    )
)
FROM public.device AS d
